...
# ── f_intraEU_final ───────────────────────────────────────────────────────────
print("Building f_intraEU_final …")
...
# ── Employment: NACE → CPA mapping ───────────────────────────────────────────
print("Building Em_EU …")
...
# Explicit NACE→CPA map (all 64 pairs, handling all formatting differences)
nace_to_cpa_map = {
...
print(f"  NACE→CPA map size: {len(nace_to_cpa_map)} (expected 64)")
assert len(nace_to_cpa_map) == 64

# Check all NACE codes in data are mapped
nace_in_data = set(df_emp['nace_r2'].unique())
unmapped = nace_in_data - set(nace_to_cpa_map.keys())
if unmapped:
    print(f"  WARNING: Unmapped NACE codes in data: {unmapped}")
else:
    print(f"  All NACE codes in data are mapped ✓")
...
# Check for missing countries per code
em_series = df_emp_clean.groupby('row_label')['OBS_VALUE'].sum()
em_EU = em_series.reindex(index_labels, fill_value=0)  # missing → 0

em_df = pd.DataFrame({'em_EU_THS_PER': em_EU}, index=index_labels)
em_df.index.name = 'country_product'

# Report missing
missing_em = [lbl for lbl in index_labels if lbl not in em_series.index]
if missing_em:
    print(f"  Employment zeros (missing data, set to 0): {len(missing_em)} rows")
    missing_countries = sorted(set([lbl.split('_')[0] for lbl in missing_em]))
    print(f"    Affected countries: {missing_countries}")

print(f"\n  Em_EU shape: {em_df.shape}")
print(f"  Em_EU total: {em_df['em_EU_THS_PER'].sum():,.1f} THS_PER")
print(f"  Em_EU min: {em_df['em_EU_THS_PER'].min():.3f}, max: {em_df['em_EU_THS_PER'].max():.1f}")
